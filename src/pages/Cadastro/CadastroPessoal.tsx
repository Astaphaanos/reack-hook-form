...
const CadastroPessoal = () => {
  const { 
    register,
    handleSubmit, 
    formState: {errors}, //* função do useform de errors
    watch, //* assistir a senha que é colocada no input para usar no validaSenha (função)
    } 
    = useForm<FormInputTipos>(); //* tipando o Hook (useForm<FormInputTipos>) e dizendo que ele vai receber dados desse tipo
  ...
  const validaSenha = {
    ...
    tamanhoMinimo: (val: string) => val.length >= 8 || "A senha deve ter pelo menos 6 caracteres", //! verificar se o tamanho do val é maior ou igual a 8 
      ...
      <Form onSubmit={handleSubmit(aoSubmeter)}>  
        ...
        <Fieldset>
          ...
          <Input
            ...
            $error={!!errors.nome} //! props convertido em boleado (!!)
            {...register('nome', {
            ...
            //* espalhando(...split) o register, ele tem que receber um nome que tem que ser diferente para cada um
          />
          {errors.nome && <ErrorMessage>{errors.nome.message}</ErrorMessage>} {/* Usando o errors do UseForm */}
        </Fieldset>
        ...
        <Fieldset>
          ...
          <Input
            ...
            //* pattern faz com que a gente só aceite telefones desse tipo (igual ao placeholder)
          />
